...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=12542123926:+5569993733829\n' // WhatsApp ID + phone number
            + 'END:VCARD'
prefix = '!'
blocked = []
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '⌛ CARREGANDO, AGUARDE POR FAVOR...⌛',
				success: '✔️ Sucesso ✔️',
			...
			const NomerOwner = '5569993733829@s.whatsapp.net'
  ...
  client.updatePresence(from, Presence.composing) 
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
              ...
              case 'owner':
                case 'creator':
                  client.sendMessage(from, {displayname: "NINEM(NABUTO)", vcard: vcard}, MessageType.contact, { quoted: mek})
		...
		case 'spamcall':
               client.updatePresence(from, Presence.composing)
				...
				case 'wikien':
					...
					break				
				case 'ytmp3':
					if (args.length < 1) return reply('Urlnya link?')
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mencionando Todo Mundo! 〙✪══\n╠➥'+teks+'╚═〘 ninem(nabuto) BOT 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					client.sendMessage(from, '╔══✪〘 Mencionando Todo Mundo! 〙✪══\n╠➥'+teks+'╚═〘 ninem(nabuto) BOT 〙', text, {quoted: mek})
					break
                case 'tagall5':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
